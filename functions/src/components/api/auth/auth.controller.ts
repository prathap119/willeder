...
import { getUser, getUserByEmail, updateUserFields,updateUserFieldsrefresh } from '../../../models/user';
...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  try {
   ...
   const user_id= await service.createUser(email, password, name, phone, address);
   if(user_id){
...
export const logout = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await updateUserFieldsrefresh(user_id);
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (users[0].status !== 'true') throw unauthorizedException('This user is unauthorized.');
...
export const refresh = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await updateUserFields(user.user_id, { refreshToken });
